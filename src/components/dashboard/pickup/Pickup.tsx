  ...
  const refreshData = async (): Promise<any> => {
    ...
    try {
      ...
      if (fetchRentsData.data.data) {
        const dataFormatted: Rent[] = fetchRentsData.data.data.filter(x => (x.status === 'waiting_pickup'))
          .map(x => {
  ...
  return (
    <>
      ...
      <div className="h-full flex flex-col mx-auto">
        {editModal && <PickupEditModal formData={formData} setFormData={setFormData} setShowModal={setEditModal} setActionResult={setAlert} setShowAlert={setShowAlert} refreshData={refreshData} />}
        {deleteModal && <PickupDropModals formData={formData} setFormData={setFormData} setShowModal={setDeleteModal} setActionResult={setAlert} setShowAlert={setShowAlert} refreshData={refreshData} />}
        <SearchBar wordSearch={wordSearch} onChange={onWordSearchChange} placeholder={"Cari Nama"} />
        {
          loading ?
            <svg className='w-10 mt-10 text-ws-orange animation animate-spin mt-10 mx-auto' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
