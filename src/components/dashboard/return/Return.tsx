  ...
  const refreshData = async (): Promise<any> => {
    ...
    try {
      ...
      if (fetchRentsData.data.data) {
        ...
        const dataFormatted: Rent[] = fetchRentsData.data.data.filter(x => (x.status === 'waiting_return'))
          .map(x => {
        ...
        const dataFormattedAndCalculated = dataFormatted.map((rent) => {
          ...
          if (days <= 0 && hours <= 0) {
            ...
            rent.tools.forEach(x => {
              const tool = fetchToolsData.data?.data.find(y => y.id === x.toolId);
              if (tool) {
  ...
  return (
    <>
      ...
      <div className="h-full flex flex-col mx-auto justify-start">
        {editModal && <ReturnEditModal formData={formData} setFormData={setFormData} setShowModal={setEditModal} setActionResult={setAlert} setShowAlert={setShowAlert} refreshData={refreshData} />}
        {
          loading ?
